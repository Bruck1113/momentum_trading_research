...
from sklearn_new.utils._testing import SkipTest
...
from sklearn_new.utils import as_float_array, check_array, check_symmetric
from sklearn_new.utils import check_X_y
from sklearn_new.utils import deprecated
from sklearn_new.utils._mocking import MockDataFrame
from sklearn_new.utils.estimator_checks import _NotAnArray
from sklearn_new.random_projection import _sparse_random_matrix
from sklearn_new.linear_model import ARDRegression
...
from sklearn_new.utils import _safe_indexing
from sklearn_new.utils.validation import (
    has_fit_parameter,
    check_is_fitted,
    ...
    FLOAT_DTYPES)
from sklearn_new.utils.validation import _check_fit_params

import sklearn_new

from sklearn_new.exceptions import NotFittedError, PositiveSpectrumWarning

from sklearn_new.utils._testing import TempMemmap
...
def test_suppress_validation():
    ...
    with pytest.raises(ValueError):
        assert_all_finite(X)
    sklearn_new.set_config(assume_finite=True)
    assert_all_finite(X)
    sklearn_new.set_config(assume_finite=False)
    with pytest.raises(ValueError):
        assert_all_finite(X)
