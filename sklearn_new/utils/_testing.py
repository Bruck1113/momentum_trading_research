...
import sklearn_new
from sklearn_new.base import (BaseEstimator, ClassifierMixin, ClusterMixin,
                          RegressorMixin, TransformerMixin)
from sklearn_new.utils import deprecated, IS_PYPY, _IS_32BIT
...
def assert_run_python_script(source_code, timeout=60):
    ...
    try:
        ...
        cwd = op.normpath(op.join(op.dirname(sklearn_new.__file__), '..'))
        env = os.environ.copy()
        try:
        ...
        # If coverage is running, pass the config.ini file to the subprocess
