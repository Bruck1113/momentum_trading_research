...
import sklearn_new
from sklearn_new.base import BiclusterMixin

from sklearn_new.linear_model._base import LinearClassifierMixin
...
from sklearn_new.utils.estimator_checks import (
    _mark_xfail_checks,
    _construct_instance,
...
def test_configure():
    ...
    cwd = os.getcwd()
    setup_path = os.path.abspath(os.path.join(sklearn_new.__path__[0], '..'))
...
def test_import_all_consistency():
    ...
    pkgs = pkgutil.walk_packages(path=sklearn_new.__path__, prefix='sklearn.',
...
def test_root_import_all_completeness():
    ...
    for _, modname, _ in pkgutil.walk_packages(path=sklearn_new.__path__,
                                               onerror=lambda _: None):
        if '.' in modname or modname.startswith('_') or modname in EXCEPTIONS:
            continue
        assert modname in sklearn_new.__all__
...
def test_all_tests_are_importable():
    ...
    lookup = {name: ispkg
              for _, name, ispkg
              in pkgutil.walk_packages(sklearn_new.__path__, prefix='sklearn.')}
